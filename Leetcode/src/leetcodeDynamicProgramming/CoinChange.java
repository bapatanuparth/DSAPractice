	...
	//create memo based on given condition
	  int memo[][]= new int[13][10001];
	    ...
	    public int coinChanger(int[] coins, int amount, int n) {
	        ...
	        //we have 2 choices -->
	        //1. either include current coin in the sum or exclude current coin from the sum
	        if(coins[n-1]<=amount){
	        	//we choose the min value of including the coin or excluding it
	        return memo[n][amount] = Math.min(coinChanger(coins, amount, n-1), 1+ coinChanger(coins, amount-coins[n-1], n));   
	        }
